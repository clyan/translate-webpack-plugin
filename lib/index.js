...
const TRANSFROM_SOURCE_TARGET = 'transform-source-target.txt';
 ...
 * @param {*} content 写入的内容
 */
function outputFile(content, filename = TRANSFROM_SOURCE_TARGET) {
      ...
      compilation.hooks.processAssets.tapAsync(
        ...
        async (assets, callback) => {
          ...
          for (const [pathname, source] of Object.entries(assets)) {
            ...
            const chunkSourceLanguageList = getLanguageList(
              chunkSourceCode,
              regex
            );
          ...
          try {
            ...
            if (
              chunkAllSourceLanguageStr.length !==
              chunkAllTargetLanguageStr.length
            ) {
              if (outputTxt) {
                // 出错了则输入日志
                this.writeFile(
